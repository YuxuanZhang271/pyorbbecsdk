...
def label_image(img: np.ndarray, text: str):
    """
    Draw a label with a black border and white text at the top-left of the image.
    """
    pos = (10, 30)
    ...
    while not stop_rendering:
        ...
        for i in range(curr_device_cnt):
            ...
            if depth_frame is not None:
                ...
                ).reshape((height, width)).astype(np.float32) * scale
            ...
            if color_image is not None and depth_image is not None:
                ...
                label_image(color_image, f"Device{i} Color")
                label_image(depth_image, f"Device{i} Depth")
            ...
            # Stack vertically across devices
...
def start_streams(pipelines: List[Pipeline], configs: List[Config]):
    for idx, (pipeline, config) in enumerate(zip(pipelines, configs)):
        print(f"Starting device {idx}")
        pipeline.start(
            config,
            lambda frames, curr_index=idx: on_new_frame_callback(frames, curr_index)
        )
